...
export const adminNotifikasiCreate = async (body: unknown) => {
  return api.post<ApiResponse<Notifikasi>>('/api/lpk/notifikasi', body);
};

export const adminNotifikasiUpdate = async (id: string, body: unknown) => {
  return api.put<ApiResponse<Notifikasi>>(`/api/lpk/notifikasi/${id}`, body);
};

export const adminNotifikasiDelete = async (id: string) => {
  return api.delete<ApiResponse<1 | 0>>(`/api/lpk/notifikasi/${id}`);
};

export const adminNotifikasiGetAll = async (params: unknown) => {
  return api.get<ApiResponse<PaginationData<Notifikasi[]>>>('/api/lpk/notifikasi', { params });
};

export const adminNotifikasiGetById = async (id: string) => {
  return api.get<ApiResponse<Notifikasi>>(`/api/lpk/notifikasi/${id}`);
};
