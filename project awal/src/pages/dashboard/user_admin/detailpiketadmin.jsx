...
const DetailPiketAdmin = () => {
  ...
  const { cpmiData, periodFilter = 'hari', startDate, endDate } = location.state || {};
  ...
  // Cari data CPMI berdasarkan nama dengan safety check
  const safeCpmiList = cpmiList || [];
  const foundCpmiData = cpmiData || safeCpmiList.find(cpmi => cpmi && cpmi.nama === decodedNama);
  
  // Debug log
  console.log('URL nama:', nama);
  console.log('Decoded nama:', decodedNama);
  console.log('CPMI Data found:', foundCpmiData);
  console.log('Available CPMI names:', safeCpmiList.map(c => c?.nama).filter(Boolean));

  // Generate dummy data piket berdasarkan filter
  const generatePiketData = () => {
    if (!foundCpmiData) return [];
  ...
  const handleDateClick = (tanggal) => {
    ...
    navigate(`/dashboard/admin/laporan-piket/detail/${encodedNama}/tanggal/${encodedTanggal}`);
  };
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-4 text-sm text-light-text-secondary dark:text-dark-text-secondary mb-2">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{foundCpmiData?.alamat || 'Rumah Majikan - Taiwan'}</span>
                </div>
                <div className="flex items-center space-x-1">
          ...
          {Object.keys(groupedData).length === 0 ? (
            ...
            <div className="space-y-6">
              {Object.entries(groupedData).map(([tanggal, activities], dateIndex) => (
                ...
                >
                  {/* Date Header */}
                  <div className="bg-purple-50 dark:bg-purple-900/20 px-4 py-3 border-b border-light-border dark:border-dark-border">
                        ...
                        >
                          <td className="px-4 py-4">
                            <button
                              ...
                              className="flex items-center space-x-3 text-left hover:text-purple-600 dark:hover:text-purple-400 transition-colors"
                            >
                          ...
                          <td className="px-4 py-4">
                            <button
                              onClick={() => handleDateClick(tanggal)}
                              className="flex items-center space-x-2 px-3 py-2 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/30 transition-colors"
                            >
...
export default DetailPiketAdmin;